 ...
 */
export function normalize(input: obj[], key: string) {
  return input.reduce(
...
export const getDeviceLocation = ({
  ...
  navigator.geolocation.getCurrentPosition(
    ({ coords: { latitude: lat, longitude: lon } }: obj) => {
      onSuccess({ lat, lon });
    },
    () => {
      onError();
 ...
 */
export const parseObjToSearchString = (params: obj) => {
  let paramsString = "";
